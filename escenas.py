...
st.title("Analizador de Novelas")
st.write("""
Esta aplicación analiza una novela compuesta de capítulos y escenas, buscando errores como falta de coherencia o consistencia, falta de profundidad psicológica en los personajes, descripciones demasiado largas o demasiado cortas, pobreza o repetición de vocabulario, etc.
""")
...
def dividir_en_capitulos(texto):
    """
    Divide el texto de la novela en capítulos basados en encabezados o cambios de línea dobles.
    
    Parámetros:
        texto (str): El texto completo de la novela.
    
    Retorna:
        list: Lista de capítulos divididos.
    """
    # Suponiendo que los capítulos están separados por dos saltos de línea
    capitulos = texto.split('\n\n')
    capitulos = [capitulo.strip() for capitulo in capitulos if capitulo.strip()]
    return capitulos

def dividir_en_escenas(capitulo):
    """
    Divide el texto de un capítulo en escenas basadas en encabezados o cambios de línea dobles.
    
    Parámetros:
        capitulo (str): El texto completo del capítulo.
    
    Retorna:
        list: Lista de escenas divididas.
    """
    # Suponiendo que las escenas están separadas por dos saltos de línea
    escenas = capitulo.split('\n\n')
...
def analizar_escena(escena):
    ...
    """
    prompt = f"""
    Por favor, analiza la siguiente escena de una novela. Identifica errores como falta de coherencia o consistencia, falta de profundidad psicológica en los personajes, descripciones demasiado largas o demasiado cortas, pobreza o repetición de vocabulario, etc. Proporciona recomendaciones claras y específicas para cada área de mejora. Responde en formato JSON con las siguientes claves: "escena", "issues", "suggestions".
...
def analizar_novela(texto):
    ...
    for idx_capitulo, capitulo in enumerate(capitulos, start=1):
        escenas = dividir_en_escenas(capitulo)
        for idx_escena, escena in enumerate(escenas, start=1):
            analisis = analizar_escena(escena)
            if analisis:
                analisis_por_escena.append({
                    "capitulo": idx_capitulo,
                    "escena": idx_escena,
                    "analisis": analisis
                })
            else:
                st.error(f"No se pudo obtener análisis para la Escena {idx_escena} del Capítulo {idx_capitulo}.")
...
def generar_informe(analisis_por_escena):
    ...
    for analisis in analisis_por_escena:
        informe += f"### Capítulo {analisis['capitulo']}, Escena {analisis['escena']}:\n\n"
        informe += f"**Problemas Identificados:** {analisis['analisis']['issues']}\n\n"
        informe += f"**Sugerencias de Mejora:** {analisis['analisis']['suggestions']}\n\n"
