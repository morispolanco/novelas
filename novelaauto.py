...
# Función para distribuir la trama en capítulos y escenas
def distribute_plot(plot):
    prompt = [
        {"role": "user", "content": (
            "Distribuye la siguiente trama en 12 capítulos, cada uno con 5 escenas. "
            "Proporciona una lista estructurada con capítulos y escenas en formato JSON, "
...
# Función para describir cada escena
def describe_scenes(distribution):
    prompt = [
        {"role": "user", "content": (
            "Describe detalladamente qué sucede en cada una de las siguientes 60 escenas para asegurar coherencia y consistencia en la trama. "
...
# Función para escribir una escena
def write_scene(scene_description):
    prompt = [
        {"role": "user", "content": (
            "Escribe una escena de aproximadamente 1000 palabras con las siguientes características de un thriller político: mucha acción, ritmo rápido, "
            ...
            f"Descripción de la escena: {scene_description}"
        )}
    ]
    return call_openrouter_api(prompt)
...
def create_word_document(chapters, scenes_content):
    ...
    # Estilo para las escenas
    if 'SceneStyle' not in styles:
        scene_style = styles.add_style('SceneStyle', WD_STYLE_TYPE.PARAGRAPH)
        scene_style.font.name = 'Arial'
        scene_style.font.size = Pt(14)
        scene_style.font.bold = True
        scene_style.paragraph_format.space_before = Pt(6)
        scene_style.paragraph_format.space_after = Pt(3)
    
    # Estilo para el contenido de las escenas
    ...
    for chapter_num, chapter in enumerate(chapters, start=1):
        ...
        # Iterar sobre las escenas del capítulo
        for scene_num, scene in enumerate(chapter['scenes'], start=1):
            # Añadir título de la escena
            scene_title = doc.add_paragraph(f"Escena {chapter_num}.{scene_num}: {scene['title']}", style='SceneStyle')
            
            # Añadir contenido de la escena
            scene_key = f"{chapter_num}.{scene_num}"
            content_text = scenes_content.get(scene_key, "Contenido no disponible.")
...
if 'scenes_description' not in st.session_state:
    st.session_state.scenes_description = None
if 'scenes_content' not in st.session_state:
    st.session_state.scenes_content = {}
...
if st.session_state.step == 2:
    ...
    # Paso 2: Distribuir la trama en capítulos y escenas
    status_text.text("Distribuyendo la trama en capítulos y escenas...")
    distribution = distribute_plot(st.session_state.plan)
    st.session_state.distribution = distribution
    progress_bar.progress(20)
    time.sleep(1)

    # Paso 3: Describir cada escena
    status_text.text("Describiendo cada escena...")
    scenes_description = describe_scenes(st.session_state.distribution)
    st.session_state.scenes_description = scenes_description
    ...
    try:
        ...
        scenes_description_data = json.loads(st.session_state.scenes_description)
    except json.JSONDecodeError:
        ...
        st.write("Descripción de escenas recibida:", st.session_state.scenes_description)
        st.stop()

    # Crear una estructura de capítulos y escenas
    st.session_state.chapters = []
    for chapter in distribution_data.get("capítulos", []):
        chapter_dict = {
            "title": chapter.get("título", f"Capítulo {len(st.session_state.chapters)+1}"),
            "scenes": [{"title": scene.get("título", f"Escena {i+1}")} for i, scene in enumerate(chapter.get("escenas", []))]
        }
        st.session_state.chapters.append(chapter_dict)

    # Asignar descripciones a cada escena
    for idx, scene_desc in enumerate(scenes_description_data.get("escenas", []), start=1):
        chapter_num = (idx - 1) // 5 + 1
        scene_num = (idx - 1) % 5 + 1
        scene_key = f"{chapter_num}.{scene_num}"
        st.session_state.scenes_content[scene_key] = scene_desc.get("descripción", "")

    progress_bar.progress(50)
    status_text.text("Descripción de escenas completada.")
    time.sleep(1)

    # Mostrar las descripciones de las escenas para aprobación
    st.subheader("Descripciones de las Escenas")
    st.write("A continuación se presentan las descripciones generadas para cada escena. ¿Deseas aprobarlas y continuar con la generación de las escenas?")
    
    # Mostrar las descripciones en un formato legible
    for chapter in st.session_state.chapters:
        st.markdown(f"### {chapter['title']}")
        for scene in chapter['scenes']:
            scene_key = f"{st.session_state.chapters.index(chapter)+1}.{chapter['scenes'].index(scene)+1}"
            st.markdown(f"#### Escena {scene_key}: {scene['title']}")
            st.write(st.session_state.scenes_content.get(scene_key, "Descripción no disponible."))
...
if st.session_state.step == 3:
    ...
    # Paso 4: Escribir cada escena
    status_text.text("Escribiendo cada escena de la novela...")
    total_scenes = 12 * 5  # 12 capítulos, 5 escenas cada uno
    for idx in range(1, total_scenes + 1):
        chapter_num = (idx - 1) // 5 + 1
        scene_num = (idx - 1) % 5 + 1
        scene_key = f"{chapter_num}.{scene_num}"
        description = st.session_state.scenes_content.get(scene_key, "")
        if not description:
            st.error(f"Descripción faltante para la escena {scene_key}.")
            break
        content = write_scene(description)
        st.session_state.scenes_content[scene_key] = content
        progress = 50 + (50 * idx) // total_scenes
        progress_bar.progress(progress)
        status_text.text(f"Escribiendo escena {idx} de {total_scenes}...")
    ...
    with st.spinner("Creando el documento Word..."):
        word_doc = create_word_document(st.session_state.chapters, st.session_state.scenes_content)
...
# Mostrar celebración si el proceso está completo
